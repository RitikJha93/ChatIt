...
const accessChat = asyncHandler(async(req,res)=>{
    ...
    if(!userId){
        console.log('userId param not sent with the request 400');
        return res.status(400).json({error : 'userId param not sent with the request 400'})
    }
...
const renameGroup = asyncHandler(async(req,res)=>{
    ...
    if(!updatedChat){
        res.status(400).json({error : "cannot rename the group"})
    }
    else{
...
const addToGroup = asyncHandler(async(req,res)=>{
    ...
    if(!added){
        res.status(400).json({error : "cannot add to the group"})
    }
    else{
